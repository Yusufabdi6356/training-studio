# Training Studio — Landing Page

Final HTML & CSS project: based on the [Training Studio](https://templatemo.com/live/templatemo_548_training_studio) template by TemplateMo, rebuilt from scratch with plain **HTML**, **CSS**, and a small amount of **vanilla JavaScript** — no Bootstrap, no jQuery.

## Features

- Sticky navigation bar that turns solid after scrolling
- Fullscreen hero banner with autoplay background video
- "Choose Program" section with a responsive 2-column grid
- Call-to-action banner with a fixed parallax background
- "Our Classes" section with working tabs
- "Classes Schedule" table with a day filter (Monday–Friday)
- "Expert Trainers" cards with Font Awesome social icons
- Contact section with an embedded Google Map and a styled form
- Fully responsive: desktop, tablet, and mobile with a hamburger menu

## Built With

- HTML5
- CSS3 (Flexbox & Grid)
- Vanilla JavaScript
- [Poppins](https://fonts.google.com/specimen/Poppins) font from Google Fonts
- [Font Awesome](https://fontawesome.com/) icons

## Project Structure

```
Final/
├── index.html
├── css/style.css
├── js/main.js
├── assets/images/
└── screenshots/
```

## Screenshots

### Hero Banner

![Hero banner](screenshots/01-hero.png)

### Choose Program

![Choose Program section](screenshots/02-programs.png)

### Call to Action

![Call to action banner](screenshots/03-cta.png)

### Our Classes (Tabs)

![Our Classes section with tabs](screenshots/04-classes.png)

### Classes Schedule (Day Filter)

![Classes schedule table](screenshots/05-schedule.png)

### Expert Trainers

![Expert trainers cards](screenshots/06-trainers.png)

### Contact (Map + Form)

![Contact section](screenshots/07-contact.png)

## Responsiveness (Mobile 375px)

| Mobile Hero | Mobile Menu | Mobile Schedule |
| :---: | :---: | :---: |
| ![Mobile hero](screenshots/09-mobile-hero.png) | ![Mobile menu open](screenshots/10-mobile-menu.png) | ![Mobile schedule](screenshots/11-mobile-schedule.png) |

## Full Page

<details>
<summary>Click to view the full desktop page</summary>

![Full page](screenshots/08-full-page.png)

</details>

## Credits

- Original design: [TemplateMo 548 Training Studio](https://templatemo.com/tm-548-training-studio)
- Images and video: [Pexels](https://www.pexels.com)
